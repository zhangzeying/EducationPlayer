...
/** <点击了底部工具条上的播放/暂停按钮> */
- (void)cyVideoToolBarView:(BOOL)isPlay
{
    if (isPlay) {
        self.toolBarView.playBtn.selected = !isPlay;
        [self.player play];
        self->_playing = YES;
    }else{
...
- (void)goForwardPlay {
    ...
    self.toolBarView.playBtn.selected = !self.playing;
    NSString *str = self.toolBarView.playBtn.selected ? @"1" : @"0";
    [[NSNotificationCenter defaultCenter]postNotificationName:@"PlayStateChange" object:str];

    double currentTime = (double)(self.toolBarView.playerSlider.value) + 4.f;
//    NSTimeInterval duration = CMTimeGetSeconds(self.player.currentItem.duration);
//    self.toolBarView.timeLabel.text = [self stringWithCurrentTime:currentTime duration:duration];
    [self.player seekToTime:CMTimeMakeWithSeconds(currentTime, NSEC_PER_SEC) toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero];
